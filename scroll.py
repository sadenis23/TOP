    ...
    if is_personal_info_filled() and not st.session_state.is_form_submitted:
        ...
        if st.button("Baigti pildyti formą ir siųsti duomenis"):
            ...
            if missing_fields:
                ...
                with st.spinner("Palaukite, duomenys siunčiami į duomenų sandėlį..."):
...
# -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if st.session_state.get('current_step', 1) >= 2:
        # Improved JavaScript to save and restore scroll position
        scroll_script = """
        <script>
            ...
            // Restore the scroll position after the page reloads with a small delay
            document.addEventListener('DOMContentLoaded', function() {
                ...
                }, 50); // Delay ensures DOM is fully rendered before restoring position
            });
        </script>
        """
        ...
        with st.expander("2. Duomenų šaltiniai", expanded=False):
            ...
            # Render data sources dynamically
            for i in range(st.session_state['data_sources_count']):
                ...
                with col1:
                    st.selectbox(
                        ...
                        key=f"type_{i}",
                        index=0 if st.session_state['data_sources'][i]['type'] == "" 
                        else st.session_state['data_sources'][i]['type']
                    )
                with col2:
                    st.text_input(
                        f"Serveris/Duomenų bazė/Schema/Lenta/Pavadinimas", 
                ...
                # Code-related input
                st.text_area(
                    f"Komentaras apie šaltinį ir jo ypatumus", 
                ...
                st.text_input(
                    f"Naudinga nuoroda (pvz. Gitlab nuoroda į atliktas šaltinio transformacijas)", 
                    placeholder="Pridėkite šaltinio naudingą nuorodą", 
            ...
            if st.button("Pridėti naują duomenų šaltinį", key="add_new"):
                add_data_source()

            st.markdown('<div class="center-button">', unsafe_allow_html=True)
            if st.button("Tęsti", key="section2"):
                st.session_state['attempted_section'] = st.session_state.get('attempted_section', {})
